...
class GroupMessageConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        self.room_group_name = 'chat_%s_group' % self.room_slug
       
        self.thread_check = await self.check_thread(self.room_slug)

        if self.thread_check == False:
            await self.save_thread(name=self.room_slug)
    ...
    @database_sync_to_async
    def check_thread(self, slug):
        if Threads.objects.filter(slug=slug).count() > 0 :
         return True
        else:
            return False
